...
async fn handle_add(
    ...
    for (role_id, role_name) in [
        ...
        // as the new "trusted" state. Name and position are already captured
        // for every role regardless of registration, so re-writing them here
        // is harmless, and keeps this call a single, simple "backfill
        // whatever's missing (i.e. permissions, for a role with no prior
        // baseline at all)" step.
