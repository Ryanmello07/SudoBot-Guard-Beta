use crate::guard::{baseline, name_drifted, permission_drifted, reaction};
...
pub async fn run_once(ctx: &Context, pool: &PgPool, guild_id: GuildId) {
    ...
    //    carbon-copy check, registered or not, reverting just the one
    //    changed role each time (each is independent, so that never
    //    perturbs another role). Skipped entirely when lockdown is off —
    //    only manual-grant reversion and role recreation (steps 2-3 below)
    //    stay active regardless of lockdown state.
    if lockdown_enabled {
        ...
        // Position is guarded absolutely too (every role below the bot),
        // but as ONE bulk correction for the whole guild rather than per
        // role — see the design note on `crate::guard::position_drifted`
