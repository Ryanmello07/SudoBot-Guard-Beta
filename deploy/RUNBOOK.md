...
4. **Restore Postgres** from the last known-good backup (`deploy/restore.sh`, targeting a timestamp before the suspected compromise). **Before running it, check the newest base backup's own timestamp under `/var/backups/sudobot-guard/base/`.** `restore.sh` always restores from the newest base backup on disk, regardless of your target time -- so even if an older, clean base backup is sitting right next to it (retention keeps 2), the tool will still restore onto the newer one. Only proceed if that newest backup's timestamp itself predates the compromise window; if it doesn't, this tool cannot produce a clean restore as-is.
